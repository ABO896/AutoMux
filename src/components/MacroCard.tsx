...
// D-13/D-14 (UX-09/UX-08): one macro card — normal display, full inline
// expand-in-place edit, and inline delete confirmation (C-D1, replaces
// window.confirm()). Signals-down / accessor-props pattern (RESEARCH.md
// Pattern 1) — every signal driving this card (editingCardId,
// confirmingDeleteId, the edit-form field values) is owned by App() and
// passed down as accessor/callback props; MacroCard has no local reactive-
// state primitives of its own for edit/confirm state, which would silently
// break the existing single-card mutual-exclusion invariant.
        ...
        fallback={
          <div class="flex items-center justify-between gap-3">
            <div class="flex-1 min-w-0">
              <p class="text-[15px] font-semibold text-text-main flex items-center min-w-0">
                <span class="shrink-0">Delete "</span>
                <span class="truncate min-w-0" title={props.macro.name}>
                  {props.macro.name}
                </span>
                <span class="shrink-0">"?</span>
              </p>
              <p class="text-[13px] text-text-dim mt-1">This can't be undone.</p>
            </div>
            <div class="flex items-center gap-1.5 shrink-0">
              <button
                onClick={() => props.onDeleteCancel()}
                class="px-2.5 py-1 rounded-md text-[11px] font-semibold bg-surface-alt border border-border text-text-muted hover:text-text-main transition-colors cursor-pointer"
              >
                Cancel
              </button>
              <button
                onClick={() => props.onDeleteConfirm()}
                class="px-2.5 py-1 rounded-md text-[11px] font-semibold bg-danger/10 text-danger border border-danger/20 hover:bg-danger/20 transition-colors cursor-pointer"
              >
                Delete
              </button>
            </div>
